...
//   [count]          路由上 `svg text` 数量 ≥ 期望(期望 = 该文件模板里 SvgText 用法的静态个数;v-for 承载的写 min + whyMin 覆盖;
...
//   [invisible]      看得见:自身与祖先(到 body)computed visibility 均 visible、opacity 均 > 0;fill 不透明(alpha > 0 且非 none)、
//                    fill-opacity > 0(A1 P0-3:此前只判「排版了」不判「看得见」)。
...
//                    计算值必须等于属性值(attributify / 任何 CSS 劫持都在这里现形;A1 P0-2 的 :opacity 就是这条抓)。
...
//                    候选谓词与源码门共用 lib,A1 P1-2)。
//   [console]        路由上 console error = 0,且 **Vue warn**(未解析组件 / 未知 custom element)= 0(A1 P2-6:忘 import 时 Vue 只发 warn)。
//
// 已知边界(如实,不装作守住了):字色与背景同色 / 被别的层盖住 / 两个标签互相重叠 —— 属对比度与遮挡问题,由走查 tester 与
// a11y 工具判,本门不判(判「盖住」会被 mock 的里程碑弹层弄成随机红)。
//
...
export function snapshotScript() {
  return () => {
    const alpha = (c) => { const m = String(c).match(/rgba?\(([^)]+)\)/); if (!m) return c === "transparent" || c === "none" ? 0 : 1; const p = m[1].split(/[,/\s]+/).filter(Boolean); return p.length >= 4 ? parseFloat(p[3]) : 1; };
    ...
    for (const el of document.querySelectorAll("svg, svg *")) {
      ...
      for (const [name, f] of Object.entries(ATTRS)) {
        ...
        if (v == null || !/^\s*[0-9.]+(px)?\s*$/.test(v)) continue; // 只判纯数字/px 的属性值(百分比、关键字不判)
        const [want, got] = f(v, cs);
        if (Number.isFinite(want) && Number.isFinite(got) && Math.abs(want - got) > 0.01) hijacked.push(`<${el.localName} ${name}="${v}"> 计算值 ${got}`);
      }
    }
    return { texts, uniTextInSvg: document.querySelectorAll("svg uni-text").length, hijacked: hijacked.slice(0, 8), hijackedCount: hijacked.length, doc: { w: document.documentElement.scrollWidth, h: document.documentElement.scrollHeight } };
  };
}
...
async function main() {
  ...
  for (const [file, cfg] of Object.entries(ROUTE_MAP)) {
    ...
    else console.log(`PASS ${cfg.route}  svg text ${snap.texts.length}(≥${min})· 全部 bbox>0 · 可见 · 属性未被劫持 · uni-text 0 · console 0 —— ${snap.texts.map((t) => t.text).join(" / ")}`);
  }
  await browser.close();
