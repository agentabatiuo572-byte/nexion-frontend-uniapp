...
const LEDGER = {
  ...
  "/api/app/security/password/commands/:param": "BACKEND: AppUserSecurityController GET /password/commands/{commandKey};authenticated password-command receipt recovery",
  ...
  "/api/app/trade-in/capacity-keep": "TBD-NAME: 同上(保留现有容量并购买新设备)",
  "POST /api/app/trade-in/capacity-keep": "BACKEND: AppDeviceE3Controller 已实现;容量保留购买命令,前端 PRD 待同步",
...
// ── 2026-08-14 并入远端主线(9db1d5d)带来的新端点 ────────────────────
  ...
  "/api/app/support/ai/handoffs": "BACKEND: AppNovaAiController POST /handoffs;authenticated human-support handoff with Idempotency-Key",
  ...
  "/api/orders/:param/pay": "BACKEND: 已创建商城订单只允许 NexGrid 钱包同步扣款并激活;HDPay 只用于钱包充值,前端 PRD 待同步",
