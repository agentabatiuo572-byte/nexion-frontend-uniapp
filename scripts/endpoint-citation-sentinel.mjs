...
const LEDGER = {
  ...
  "POST /api/me/password": "TBD: PRD 未定义改密 endpoint(§4.8 只描述规则)",
  ...
  "GET /api/config/fx": "TBD: PRD 未定义牌价配置 endpoint([D6] 单源,候选名)",
  "GET /api/config/genesis": "TBD: PRD 未定义 genesis 配置 endpoint(§10.2 只有 GET /api/genesis/state)",
  "GET /api/config/gpu-tiers": "TBD: PRD 未定义显卡档位 endpoint(admin E6 侧已实现,前端 PRD 待补)",
  "GET /api/config/release-gates": "TBD: PRD 未定义上架节奏门 endpoint(后台 E1 配置项)",

  // ── genesis ───────────────────────────────────────────────────────────
  "POST /api/genesis/secondary/fulfill": "PRD §10.2.4",
  "POST /api/genesis/{list,unlist}": "PRD §10.2.4",
  "GET /api/genesis/eligibility": "TBD: PRD 未定义创世资格 endpoint(§10.2 只有 state)",
  "POST /api/genesis/invite/redeem": "TBD: PRD 未定义创世邀请码核销 endpoint",
  ...
  "GET /api/store/catalog": "TBD: PRD 未定义商品目录 endpoint(§7.1 只描述 SKU 数据)",
  ...
  "POST /api/withdrawals/eligibility": "TBD: PRD 未定义提现资格预检 endpoint(K3 判定,候选名)",
  ...
  "POST /api/rebind-orders": "TBD: PRD 未定义换绑单 endpoint(§9.9 只描述 $1 验证转账规则)",
  ...
  "POST /api/me/milestones/:id/claim": "TBD: PRD §9.11c.1 只定义 GET /api/config/milestones,领取 endpoint 待补",

  // ── events / lucky spin ───────────────────────────────────────────────
  "GET /api/events": "PRD §11.11",
  "POST /api/events/:id/spin": "PRD §11.11",
  "POST /api/events/:id/join": "TBD: PRD 未定义活动报名 endpoint(§9.11e 原子模式候选)",
  "POST /api/events/:id/claim": "TBD: PRD 未定义活动领奖 endpoint(§9.11e 原子模式候选)",

  // ── devices / heartbeat / onboarding ──────────────────────────────────
  "POST /api/device/:id/heartbeat": "PRD §6.11 / §12.2(PRD 原文标『endpoint TBD,候选名』)",
  "GET /api/devices/eligibility": "TBD: PRD 未定义购机资格 endpoint(候选名)",
  ...
  "GET /api/leaderboard": "TBD: PRD 未定义排行榜 endpoint(§11.10 只描述榜单口径)",
  "GET /api/config/exchange/caps": "PRD §9.11c.1",
  "POST /api/admin/exchange/pause": "PRD §9.11d",
  "GET /api/network/members": "TBD: PRD 未定义关系网 endpoint(§8.3 只描述图数据)",
  "GET /api/network/regions": "TBD: PRD 未定义节点分布 endpoint(§5.6 地球仪装饰数据)",
  "GET /api/pool/state": "TBD: PRD §9.11 待补(leadership-pool.ts 已标 candidate)",
  "GET /api/server-time": "TBD: PRD 未定义授时 endpoint(§9.11a.4 只要求 server time 单点)",
  "GET /api/market": "TBD: PRD 未定义行情 endpoint(§10.3 只有 /api/market/nex)",
  "/api/market/nex": "PRD §10.3(WebSocket 推送)",
  "GET /api/market/tokens": "TBD: PRD 未定义多币种行情 endpoint(候选名)",
