...
| | 采纳? | 理由 |
|---|---|---|
| **内容没变不写(脏检查)** | ❌ **已撤销**(2026-08-05,实测推翻) | 曾按「砍掉 52% 空写」采纳,**真浏览器实测反过来**:判据 1176µs/拍 vs 省下 62.5µs/拍,**净亏 18.8×**。原结论出自 Node harness,那里写盘是 `Map.set` 近似免费。另:「行为中性」这句也被证伪 —— 跳过写等于连内存新值一起丢弃,而排除名单的安全性没有任何门守(红测塞进 `todayEarnings` → 钱当场少算而全绿)。详见 `2026-08-04-change2-proposal.md` 与 commit `840cc45` |
...
(原本这里写着「`updatedAt` 要排除在比对之外」等实现细节 —— 随⑤撤销一并作废。)
...
| 改动 | 状态 | 实测 / 门 |
|---|---|---|
| ⑤ 内容没变不写盘(脏检查) | ❌ **已撤销** `840cc45`(曾落地 `2a26ac7`) | 次数确实降了(60→40 次/分、降 33% 字节),但**真浏览器实测净亏 18.8×**:判据 1176µs/拍 vs 省下 62.5µs/拍。`setItem` 写 105KB 只要 96.7µs —— 写盘从来不是贵的那头。配套焊门 `selfcheck-snapshot-write.mjs` 禁止重来 |
| ⑤ 时间节流 | ❌ **不做**(证伪毙掉) | `settle()` 同拍还写两张不节流、且台账无幂等键的表 → 会让释放引擎重复计数(实测释放拍损失 0.20 NEX / 120 秒台账虚记 44%) |
| ③ 三处字面量字段集一致门 | ✅ **已落地** `e02d50a`(**先于负载**) | 三处 + 类型声明四方各 8 字段一致;红测:只给 persist 加 `bills` → 门精确点名 `-bills`。verify 408 → 409 |
| ② 让「钱+收据」一次落盘 | ⏳ 下一步,**待证伪** | 回源后**收窄**:不搬内存归属(会踩 P-031),改为掐掉 `bills.persist()` 的独立写腿 + 快照带 `bills`,由已被机器门焊死的收据咽喉 `money-receipt.ts` 合成一次写。提案 `2026-08-04-change2-proposal.md`,依赖 ③ 的门已就位 |
| ① `applyMoneyTx` 单次事务 | ⏳ 待 ② | |
| ④ 跨账号窄原语走 CAS | ⏳ 待 ② | |

### 脏检查这一轮真正的教训(2026-08-05 撤销后补写)

**我量错了对象。** 我量的是「写盘次数少了多少」,没量「一次写盘到底值多少钱」,
也没量「判据自己要花多少钱」。真浏览器一测:`setItem` 105KB 只要 96.7µs ——
**它从来不是贵的那头**,而我为了省它每拍多花 1176µs。
**频次高 ≠ 成本高**;正确的判据是 `代价 × 全部次数 < 收益 × 命中次数`,两边都要有实测数字。

**我的自证探针也不够。** 我只查了自己最怀疑的那个方向(内存心跳会不会被回退),
测出「回退 0 次、最大滞后 1 秒 vs 180 秒判定窗口」就收手了 —— 数字没错,但那是**一个字段的特例**。
独立 agent 把同一机制推到一般情形:往排除名单里塞一个金额字段,
**钱当场少算(10 而非 25),而当时全部哨兵照样绿**。
即:这类优化真正的危险不在慢,在于它给「静默丢数据」开了一条零阻力通道。

已沉淀进 memory `feedback_optimization_harness_bias`。

### 撤销前记录的两条实证(保留作史料)

1. **第一版完全无效**(排除 `updatedAt` 后仍 60 写 60)。逐拍 diff 才看到:即使某拍**一分钱
   收益都没有**,`lastBucketedAt` 与 `onlineHeartbeatAt` 仍被盖成当前时刻。
   这两个字段只表示「这一拍跑过了」,故一并排除;**`lastSettledAt` 不排除**——它是计价锚点。
2. **我最怀疑的那条被自己的探针否掉**:担心 `stored = latest` 被 adopt 回内存会让心跳每拍回退。
   实跑 8 拍:**回退 0 次**,内存心跳最大滞后 **1 秒**,而在线判定窗口是 **180 秒**。
   原因是跳过写的那一拍,`latest` 与内存本就只差被排除的那两个戳。

### 本轮的工具坑(记一笔,别再踩)

红测备份写 `/tmp/xxx.bak` —— **bash 的 `/tmp` 与 Node 的 `/tmp` 在 Windows 上不是同一处**
(后者解析成 `D:\tmp`)。结果是备份根本没写成,而随后的 `cp` 拿了个**旧文件**把 `account-cloud.ts`
覆盖坏了(少 145 行)。已从 HEAD 干净还原重做。
⇒ **红测备份一律用工程内相对路径**,临时文件按铁律移进 `.trash/` 而非硬删。
