...
| 测什么 | 脚本 | 结果 |
|---|---|---|
| 落盘代价基线 ⚠️ **只量了次数没量耗时,据此得出的优化 2026-08-05 被实测推翻** | `scripts/measure-persist-cost.mjs` | 账户快照 60 次/60s · 253.5 KB;40 条种子账单序列化 6,662 B(比整份快照还大);真实产出只 29 次 → **60 次里 31 次纯空写**。<br>🔴 **「31 次空写」是真的,「省掉它们能变快」是假的**:真浏览器 `setItem` 105KB 仅 96.7µs,而判据自身 1176µs/拍 → 净亏 18.8×。**频次高 ≠ 成本高**;基线要量耗时,不能只量次数 |
