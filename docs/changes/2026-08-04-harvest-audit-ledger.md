...
| 原编号 | 缺陷 | 修法 | 红测 |
|---|---|---|---|
| A-P1-4 | 三针**不锁同一条失败分支**,全文件共现即过(三针分塞三个无关函数、冲正分支一针没有,判据仍 true) | 按括号配对抠出**这一次调用的完整实参块**(跨行),要求 `restoreTo` 真落在某次 `postMoneyBill` 实参里 | 把 restoreTo 挪出实参(仍在文件里)→ 判红 |
| A-P1-1 | `stake-sheet` 只上 1 针,删 `{restoreTo}` 门照样绿 | 补成三针 | 删 restoreTo → 判红 |
| A-P1-2 | WIRED 是手工名单,**无「新调用点必须入册」反向门**,两道门同时失明 | 加反向入册门:全站动钱/记账的文件必须 ⊆ WIRED ∪ ALLOW;名单跨块用模块级变量传递并带空转守卫 | 造册外动钱文件 → 判红并点名 |
| P0-2→P1 | 三处状态回滚 commit 返回值全丢,失败被静默吞掉 | 接住返回值 → 走**既有的** `reportStuckFunds`(交易号 + 待对账队列) | 摘掉一处消费 → 判红并报「回滚 3 / 消费 2」 |

**机器门**:`selfcheck-money-receipt` 80 → **81 pass**;`selfcheck-deposit-resume` 10 → **17 pass**;
verify **407 pass / 0 fail**;`vue-tsc` 0 错。

---

## ⑦ 结算:不依赖主人拍板的 confirmed P0/P1 已全部修完

| 状态 | 项 |
|---|---|
| ✅ 已修(10 组) | B-P0-1 路由任务奖励零账单 · A-P0-1/3/4 判据三族逃逸 · A-P1-3/5 注释哄绿哄红 · A-P1-6 分支无正控 · A-P1-1 补针 · A-P1-2 反向入册 · A-P1-4 分支绑定 · P1-2 刷新即卡死入金(+三处失败不重排)· B-P1-5 转盘假凭证 · B-P1-2 两处「已退款」假话 · P0-3→P1 银行轨黑名单 · P0-2→P1 回滚静默 |
| ⏸ 等主人拍板 | **M-1 账单跨标签页丢失(P0)** —— 修法与完全版 A 的存储路线**同一件事**,分开做要白做两遍 |
| 📋 P2 批次(不阻断) | B-P1-3/4 降 P2(对抗**推翻**了「回归」断言:git log 证明改前更差)· B-P1-6 降 P2(书面裁决过的取舍,只差一处 `reportStuckFunds` 调用)· 单元1 的 P2×7 · 单元2 的 P2×8 · 单元3 的 P2×7 |
| ❌ 对抗推翻 | P1-4 卡轨 ref 不稳定(语义读反了:每次提交 = 一次新授权,两次提交本就是两笔真扣款;且重放无路径) |

**审计闸口径**:confirmed P0=0、confirmed P1=0(M-1 除外,它被显式挂起等待路线决定,不是遗漏)。

---

## ⑧ 存量缺陷专项轮(主人 2026-08-04 拍板「存量缺陷修完」)

### 机器门 P2 批次

| 项 | 修法 |
|---|---|
| 样本量写死 `primitives:5` 而实跑 4 | 改从 `PRIMS.length` 取(这脚本自己别处正反对写死数字) |
| 扫描后缀只 `.ts\|.vue` | 补 `.tsx/.nvue/.js/.mjs`;`.jsx/.cjs/.mts/.cts` **响亮报错**(静默跳过 = 永久隐形) |
| ⑤ 的裸调禁令只禁 `bills.add(` | 复用 ⑥ 的完整识别 |
| 4 个脚本死导入 | 清掉,逐个复跑 54/108/28/55 全绿 |
| **红测抓出的新逃逸族** | `import { useBills as X }` + `X().addMany(` 零成本逃逸 → 补进 receiver 发现 + 立成常驻正控(10 → 11 条) |

### 业务侧

| 缺陷 | 修法 | 门 |
|---|---|---|
| **领奖族「领了没发 / 发两次」**(5 处) | 收口点新增 `postMoneyBillsOnce`(按 ref 判重的多腿出口);可反序的 4 处调头成「先发钱后消费资格」+ ref 换稳定标识;反不过来的 2 处(签到/里程碑,金额由消费动作自己摇出)配自愈补发 | 新增 `selfcheck-claim-idempotency`(9 断言,接进 verify) |
| 质押失败不入待对账 | `"failed"` 走 `reportStuckFunds`;`"stuck"` 不重复入队 | — |
| 冲正分录与原分录**共用幂等键三元组** | 四处统一加 `-REV` 后缀 | — |
| 银行轨 `createdAt` 记入账时刻 | `DepositIntent` 加 `createdAt`,建单写 + 入账透传 + 存量单读时升级 | — |
| 卡轨无重入守卫 | 守卫提到函数第一行(渲染层拦截挡不住同 tick 双击) | — |
| 四处注释与代码不符 | 「资金已还原」对 `stuck` 不成立 → 改准;tradein 三处两步注释 → 单次落盘 | — |

### 本轮的两次判据自伤(都被红测抓出,已修)

1. 新门的顺序断言**不剥注释** → 把文件头注里的示例调用当成真代码,报两个假阳性。
   ⇒ 抽 `scripts/lib/strip-code.mjs` 共用(同一个坑在两道门里各咬一次)。
2. ref 稳定性判据写 `ref:[^,}]*` → 模板串 `${ev.id}` 自带 `}` 把字符类挡住,**真·假幂等一个都抓不到**。
   ⇒ 改成取到行尾再判,复跑红测确认能抓。

**⇒ 元教训**:红测不是走过场 —— 本轮**两条新判据第一次跑都是无效的**,都是红测揪出来的。

### 机器门总数

`verify` **407 → 408 pass / 0 fail**;`vue-tsc` 0 错。
...
派独立 agent 逐条核实这一轮 13 项修复(要求它自己构造逃逸样本试新判据、通读全部 diff 找
「修复自身引入的新缺陷」)。结论:**已修 10 / 半修 2 / 没修 0 / 新问题 1**。

### 验收独立复现了 main 的实景发现(两条独立路径)

自愈补发会二次发钱 —— main 在浏览器实测(跨标签页账单被覆盖 → 余额 11940→11943),
验收用探针实测(**存量分录 ref 格式不同**:老的是 `STREAK-D7-LQ8ZK1`、签到那条根本没有 ref
→ 判重命中不了 → NEX 500→517)。**理由不同、结论一致**,撤销是对的。

### 验收挖出的门空转(最要命 —— 它会藏住其余所有东西)

| 门 | 空转形态 | 修法 |
|---|---|---|
| 资金门判据 | **10 条逃逸**:改名导入 / 局部别名 / 先声明后赋值 / 多声明符 / 计算成员 / **解构改名**(拿别名去比方法名,永远比不中)/ 方法引用 / 本地 wrapper / 可选调用 / 形参注入。其中两族是**正常人会写出来的形状** | 判据**翻转**:不再枚举 receiver 名字(开放集合),改判「文件与 bills 有关 ⇒ 任何写入调用都算」。有意过近似,代价由 ALLOW 精确条数兜住。正控 11 → 17 条 |
| 领奖门 ④ | 对 hero / achievements **完全空转** —— 那两处 drafts 建在调用之外,扫实参块 `ref:` 命中 0 次 | 改扫整文件 + **空集断言**;再改仍不红(ref 是变量,时间戳藏在变量定义里)→ 两头都看 |
| 领奖门 ⑤ | 只比**第一次出现**,`onClaimBonus` 从未被检查;改成逐处配对后仍不红(跨函数就满足)→ 限定同函数体;收紧后误报折扣券路径 → 再收一层(该函数体内有发钱调用才纳入配对) |

### 验收挖出的业务侧

| 项 | 处置 |
|---|---|
| quest 族三处与领奖族同型却漏改(App.vue / share.ts / wallet-cards-new) | 统一:先发钱(幂等)→ 后消费资格,ref 换任务 id,奖励改从静态表取 |
| 成就前置门**弱于消费门**(`isClaimed` 只查了 `claim()` 守卫的一半) | 补 `isUnlocked`。当前不可达,但**渲染层的拦截不是判定层的拦截** |
| 质押两条**互相矛盾**的提示同时弹 | 收口点加 `silentFailure`,由调用方交底 |
| `types.ts` 注释被挤坏 | 拆开归位 |
| `-REV` 印给用户看 | **判定不改**(冲正行与原行并排时同号才叫人分不清),判断写进代码 |

### 🔴 本轮的元教训:红测抓出**四次**判据自伤

顺序门不剥注释 · ref 判据被 `${}` 里的 `}` 挡住 · ref 判据看不见变量定义 · 顺序门跨函数就满足。
**四次都是「第一次跑抓不到目标」**,且都不是逻辑写错,是判据自己的语法/语义边界。
⇒ **新写的判据默认无效,直到红测证明它能抓**;验收标准不是「跑绿」,是「注入真实缺陷形态后变红」。
(已追加进 memory `feedback_sentinel_green_without_checking`。)

### 机器门

`verify` **408 pass / 0 fail**;`vue-tsc` 0 错。领奖门覆盖 5 → **8 个调用点**。
