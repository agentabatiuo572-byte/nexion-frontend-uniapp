...
`App.vue` 的 `checkAuthGuard()`（每 1s 路由 tick + onShow 触发）是路由级守卫：读 `useAuth()`，访问受保护路由时只有 `!isAuthenticated` 才会 `reLaunch` 到 `pages/onboarding/intro`。已有账号登录后不再用 `onboardingComplete` 触发注册或手机算力流程；该字段只保留为服务端手机激活事实，未激活能力与奖励继续由服务端业务写入门失败关闭。无壳页（`pages/onboarding/`、`login/`、`register/`、`ref/`、`tx/`）在白名单内不拦截，避免漏斗自跳死循环；reLaunch 前 `checkQuestRoute` 会 bail，不给被踢用户误派任务奖励。

**demo 默认**：`auth` store 默认 `{isAuthenticated:true, onboardingComplete:true}`，所以守卫在常规使用中休眠、应用直接进首页；只有显式 `signOut()` 后任意受保护路由才会被 reLaunch 到登录前引导。

真后端接入：`GET /api/auth/session` 提供真实认证会话与 `onboardingComplete` 激活事实；认证会话决定能否进入主应用，激活事实不能反向把登录变成注册流程。
