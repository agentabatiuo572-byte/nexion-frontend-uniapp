...
>
> **实现面**：`Nexion-uniapp`（uni-app + Vue 3 + Pinia）。文档以 uniapp 现状为准。真相源 `Nexion-prototype`（Next/React）是迁移来源，逻辑一致。
>
...
1. **全局统计抖动**：`activeDevices`、`paidToday`、`todayIncrement` 加小随机量（纯展示用）。
...
sequenceDiagram
    ...
    Link->>Link: onLoad ?code → pickSponsor → 展示赞助人+$5+20NEX 礼
    Link->>Reg: 点 CTA → navigateTo register?ref=encode(code)
    Reg->>Reg: onLoad ?ref → sponsorPreview (注册页顶部赞助卡)
    Reg->>Spon: finish() → bind(code) (首个赞助人 wins)
    Spon->>Spon: claimGift() (一次性)
    Reg->>App: creditBalance($5) + creditNex(20) + 写 2 条 bonus 账单
    Reg-->>Reg: toast "+$5 + 20 NEX · Sponsored by {name}"
```

- 落地页 `ref/code.vue` 读 `?code=`（源 [code] 动态段）；CTA 把 code 透传给 register（`?ref=`）/login。
- 注册 `finish()`：`refFromUrl || invite`（手填邀请码）→ `sponsorship.bind`（**首个赞助人 wins，不覆盖**）→ `claimGift()`（一次性，$5 + 20 NEX）→ 入账 + 写账单。
