...
- **已转哨兵**:`scanFullScreenScrims` 三种写法全扫;新增 `BAND_ANCHOR` 钉住这三处「必须一直在扫描面内」(与 `SCRIM_EXEMPT` 极性相反:那张是「别管这些」,这张是「这些必须在管辖内」)—— 因为**光断言数值挡不住形态回退**:把消息抽屉的 `position` 改回 `absolute`,判据就整块失明,z 掉回 110 也全绿。红测 18 → 26 靶,新增 4 条轴各一靶(内联写法 / JS 对象写法 / 形态回退 / 锚定文件消失)+ 三处数值回退各一靶。⚠️ 顺带修掉一处**红测自己的盲区**:「扫描面塌空」那一靶原先用 `scanAll`(只认 CSS 的 `z-index:`)挑要剔除的文件,对 JS 对象的 `zIndex:` 是瞎的 → 带根本塌不空,这一靶测不到它想测的东西,已改用 `scanFullScreenScrims` 本身。2026-08-17。
