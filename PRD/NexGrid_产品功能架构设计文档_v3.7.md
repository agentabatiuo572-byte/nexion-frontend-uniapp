...
首次注册进入估算页，先读取当前账号与设备的服务端校准结果；仅在明确返回“尚无校准记录”时，自动采集 H5 可获取的型号、内存、核心数、屏幕、WebGL、供电及联网信息，调用服务端校准接口计算并保存。浏览器不提供的参数原样保留为空，由服务端采用保守默认值参与估算，不因参数缺失显示检测失败。页面展示服务端返回的档位、TOPS、日收益及 PC 配置的设备收益对比。
...
解释采集范围：设备提供时记录型号、内存、核心数、图形渲染器，以及联网、供电状态。H5 未提供的信号显示为空；不声称已完成 NPU 原生跑分、跨区域 ping 或散热测试。CTA 为“计算手机算力”。
...
展示服务端 score、tier、TOPS、baseRateUsdt 和可用的原始网络/供电观测；未知观测显示“—”，不把未知状态标为检测通过。任务规则提示与显式启用按钮随结果展示。所有数值均以当前账号和设备的服务端事实为准。
