    ...
    <view class="block w-full text-left relative overflow-hidden rounded-2xl active:scale-[0.998]" :style="rootStyle" @click="goStore">
      ...
      <view class="relative p-4">
        ...
        <view class="flex items-center justify-between gap-3">
          <view class="font-mono-tabular inline-flex items-center gap-1.5 min-w-0" :style="eyebrowStyle">
            <text class="truncate" style="color: var(--v5-brand)">{{ t.home.slotsHintPotentialDaily }}</text>
          </view>
        ...
        <view class="mt-2 flex items-baseline gap-2 flex-wrap">
          <view class="flex items-baseline tabular-nums" style="font-family: var(--font-amount); letter-spacing: -0.024em; line-height: 1">
        ...
        <text class="mt-2 block" style="font-size: 12.5px; line-height: 1.35; color: var(--v5-ink-3)">
          {{ slotsFormulaLead }}<text v-if="showMultiplier"> · {{ multiplierPrefix }}<text style="color: var(--v5-tech-cyan); font-weight: 600">{{ multiplierText }}×</text>{{ multiplierSuffix }}</text>
        </text>

        <!-- Slot grid -->
        <view class="mt-4 grid grid-cols-6 gap-1.5 justify-items-center">
          <view
            v-for="(slot, i) in slotCells"
            :key="i"
            :style="slotTileStyle(slot)"
            class="relative grid place-items-center"
            :class="slot.kind === 'filled' && slot.deviceKind === 'phone' ? '' : 'overflow-hidden'"
          >
            <!-- filled real device -->
            <template v-if="slot.kind === 'filled'">
              <view v-if="slot.deviceKind === 'phone'" class="relative" style="width: 44px; height: 44px">
                <image :src="PHONE_DEVICE_IMAGE_SRC" mode="aspectFit" style="width: 44px; height: 44px; display: block" />
                <view aria-hidden class="absolute" style="top: 0; right: 0; width: 6px; height: 6px; border-radius: 50%; background: var(--v5-success); box-shadow: 0 0 6px var(--v5-success); animation: v5-hb-pulse-success 2.4s ease-in-out infinite" />
              </view>
              <template v-else>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--v5-brand)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path :d="deviceIconPath(slot.deviceKind)" /></svg>
            ...
            <template v-else-if="slot.kind === 'trial'">
              ...
              <text class="absolute inset-x-0 bottom-0 text-center font-mono-tabular" style="font-size: 10.5px; line-height: 13px; background: color-mix(in oklab, var(--v5-brand-2) 26%, transparent); color: var(--v5-brand-2)">{{ t.trial.slotTag }}</text>
            </template>
            <!-- empty slot -->
            <template v-else>
        ...
        <!-- Stats row -->
        <view class="mt-4 grid grid-cols-3 text-center" :style="statsCapsuleStyle">
          <view style="border-right: 1px solid color-mix(in srgb, var(--v5-border) 62%, transparent)">
          ...
          <view style="border-right: 1px solid color-mix(in srgb, var(--v5-border) 62%, transparent)">
            <view class="flex items-baseline justify-center">
              <text class="tabular-nums" style="font-family: var(--font-v5); font-size: 15px; font-weight: 600; color: var(--v5-tech-cyan); line-height: 1; letter-spacing: -0.008em">${{ NETWORK_AVG_DAILY.toFixed(0) }}</text>
              <text class="font-mono-tabular" style="margin-left: 2px; font-size: 10.5px; color: var(--v5-ink-4); line-height: 1">/d</text>
            </view>
        ...
        <view class="mt-4 inline-flex items-center justify-center gap-1.5 w-full" :style="ctaStyle">
          <view style="width: 16px; height: 16px; transform: translateX(8px)">
          ...
          <svg style="color: var(--v5-on-brand)" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6" /></svg>
        </view>
...
const NETWORK_AVG_DAILY = 45; // mock network top tier daily (Rack P1)
...
const baseNameText = computed(() =>
  promo.value.baseKind === "phone"
    ? t.value.earn.yourPhone
    : promo.value.baseKind === null
      ? t.value.earn.noActiveDevice
      : promo.value.baseName,
);
const slotsFormulaLead = computed(() =>
  fmt(t.value.earn.slotsHintFormulaLead, {
    count: empty.value,
    name: promo.value.targetName,
    rate: `$${promo.value.targetDaily.toFixed(2)}`,
  }),
);
const multiplierPrefix = computed(() => fmt(t.value.earn.slotsHintMultiplierPrefix, { base: baseNameText.value }));
const multiplierSuffix = computed(() => fmt(t.value.earn.slotsHintMultiplierSuffix, { base: baseNameText.value }));
...
function slotTileStyle(slot: SlotCell): CSSProperties {
  if (slot.kind === "filled") {
    if (slot.deviceKind === "phone") {
      return { width: "44px", height: "44px", background: "transparent", overflow: "visible" };
    }
    return { width: "44px", height: "44px", borderRadius: "12px", background: "color-mix(in srgb, var(--v5-brand) 15%, transparent)" };
  }
  if (slot.kind === "trial") {
    return { width: "44px", height: "44px", borderRadius: "12px", background: "color-mix(in oklab, var(--v5-brand-2) 16%, transparent)" };
  }
  return {
    width: "44px",
    height: "44px",
...
const statsCapsuleStyle: CSSProperties = {
  padding: "10px 0",
  borderRadius: "20px",
  backgroundColor: "#111317",
  backgroundImage: "linear-gradient(180deg, #111317 0%, #15181C 100%)",
  border: "1px solid rgba(78, 92, 108, 0.26)",
  overflow: "hidden",
};
