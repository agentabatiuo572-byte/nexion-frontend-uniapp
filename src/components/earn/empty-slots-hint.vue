      ...
      <view class="grid grid-cols-6 gap-1.5">
        <view v-for="(slot, i) in slotCells" :key="i" :style="slotTileStyle(slot)" class="relative grid place-items-center overflow-hidden">
          ...
          <template v-else-if="slot.kind === 'trial'">
            ...
            <text class="absolute inset-x-0 bottom-0 text-center font-mono-tabular" style="font-size: 11px; line-height: 13px; background: color-mix(in oklab, var(--v5-brand-2) 26%, transparent); color: var(--v5-brand-2)">{{ t.trial.slotTag }}</text>
          </template>
