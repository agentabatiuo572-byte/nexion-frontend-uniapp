  ...
  <view v-if="show" class="mx-4">
    <view class="block relative overflow-hidden rounded-2xl" :style="rootStyle" @click="goStore">
      ...
      <view class="relative mt-3 space-y-2">
        ...
        <view>
          <view class="flex items-center justify-between mb-1" style="font-size: 10.5px">
            <text :style="ceilingAccentStyle">{{ ceilingText }}</text>
            <text class="tabular-nums" :style="ceilingAccentStyle">${{ promo.targetDaily.toFixed(2) }}/d</text>
          </view>
          <view class="h-1 rounded-full overflow-hidden" :style="targetTrackStyle">
            <view class="h-full w-full rounded-full" :style="targetBarStyle" />
          </view>
        </view>
        <view aria-hidden class="mx-0.5" :style="progressDividerStyle" />
      </view>
      ...
      <view class="relative mt-4 flex items-center justify-between gap-3">
        ...
        <view class="shrink-0 inline-flex items-center gap-1.5 active:scale-[0.97]" :style="ctaStyle">
          ...
          <svg style="color: var(--v5-on-brand)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6" /></svg>
        </view>
      </view>
    </view>
  </view>
...
const ceilingAccentStyle: CSSProperties = {
  color: "var(--v5-brand)",
};
const targetTrackStyle: CSSProperties = {
  background: "color-mix(in srgb, var(--v5-brand) 12%, var(--v5-surface-2))",
  boxShadow: "inset 0 0 0 1px color-mix(in srgb, var(--v5-brand) 14%, transparent)",
};
const targetBarStyle: CSSProperties = {
  background: "linear-gradient(90deg, #CFFF34 0%, #A9F044 42%, #55DDBD 100%)",
  boxShadow: "0 0 10px color-mix(in srgb, var(--v5-brand) 24%, transparent)",
};
const progressDividerStyle: CSSProperties = {
  height: "1px",
