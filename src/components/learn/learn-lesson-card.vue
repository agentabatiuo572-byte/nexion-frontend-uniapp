...
-->
<template>
  <view class="block relative rounded-2xl overflow-hidden active:scale-[0.99] transition-transform" :style="cardStyle" @click="open">
    <!-- left tint hairline -->
    ...
    <view style="padding: 14px">
      ...
      <view class="flex items-start" style="gap: 12px">
        ...
        <svg v-else class="shrink-0" style="color: var(--v5-ink-4); margin-top: 2px" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
      ...
      <view class="mt-3 flex items-center" style="gap: 12px">
        ...
        <view v-if="inProgress" class="flex-1 flex items-center min-w-0" style="gap: 8px">
          ...
          <text class="font-mono-tabular tabular-nums shrink-0" style="font-size: 10.5px; color: var(--v5-ink-3)">{{ lesson.durationMin }} {{ t.learn.minRead }}</text>
        </view>
