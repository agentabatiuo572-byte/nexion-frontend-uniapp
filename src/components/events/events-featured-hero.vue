  ...
  <view class="relative overflow-hidden rounded-2xl" :style="cardStyle">
    ...
    <view class="relative" style="padding: 20px">
      ...
      <view v-else-if="showJoinAction" class="mt-5 w-full rounded-full flex items-center justify-center active:scale-[0.98] transition-transform" :style="joinBtnStyle" role="button" tabindex="0" :aria-label="ev.ctaLabel ?? t.events.joinCta" @click="emit('join')">
        ...
        <svg style="color: var(--v5-on-brand); margin-left: 6px; pointer-events: none" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
...
const cardStyle = computed<CSSProperties>(() => ({
  background:
    ...
    `radial-gradient(120% 60% at 50% 130%, color-mix(in srgb, ${props.ev.tint} 8%, transparent) 0%, transparent 70%),` + `var(--v5-surface-bg)`,
