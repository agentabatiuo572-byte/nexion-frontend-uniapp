    ...
    <view style="padding: 14px">
      ...
      <view class="mt-3 flex items-center" style="gap: 12px">
        ...
        <view v-if="ev.progress" class="flex-1 flex items-center min-w-0" style="gap: 8px">
          ...
          <text class="font-mono-tabular tabular-nums shrink-0" style="font-size: 10.5px; color: var(--v5-ink-3)">{{ ev.progress.current.toLocaleString() }}/{{ ev.progress.total.toLocaleString() }}</text>
        </view>
      </view>
      ...
      <view v-else-if="showJoinAction" class="mt-3 w-full rounded-full flex items-center justify-center active:scale-[0.98] transition-transform" :style="softTintBtnStyle" @click="emit('join')">
        ...
        <svg style="margin-left: 4px" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
      <view v-else-if="ev._claimed && ev.useHref" class="mt-3 w-full rounded-full flex items-center justify-center active:scale-[0.98] transition-transform" :style="claimedUseBtnStyle" role="button" :aria-label="claimedAriaLabel" @click="onClaimedUse">
        ...
        <svg :style="{ color: ev.tint, marginLeft: '4px', pointerEvents: 'none' }" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
      <view v-else-if="ev._trackable && ev.joined && !ev._done" class="mt-3 w-full rounded-full flex items-center justify-center active:scale-[0.98] transition-transform" :style="neutralBtnStyle" @click="openHref">
        ...
        <svg style="color: var(--v5-ink-2); margin-left: 4px" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
