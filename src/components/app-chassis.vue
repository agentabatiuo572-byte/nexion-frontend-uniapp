...
<template>
  <view class="nx-chassis" style="background: var(--v5-bg)">
    <view class="nx-statusbar-space" aria-hidden="true">
      <text class="nx-statusbar-time">{{ statusTime }}</text>
      <view class="nx-statusbar-icons">
        <view class="nx-signal" aria-hidden="true">
          <i />
          <i />
          <i />
          <i />
        </view>
        <svg class="nx-wifi" width="17" height="12" viewBox="0 0 17 12" fill="none" aria-hidden="true">
          <path d="M1.4 3.8C5.4.7 11.6.7 15.6 3.8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
          <path d="M4.2 6.5c2.5-1.8 6.1-1.8 8.6 0" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
          <path d="M7.2 9.1c.8-.5 1.8-.5 2.6 0" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
          <circle cx="8.5" cy="10.7" r="1" fill="currentColor"/>
        </svg>
        <view class="nx-battery" aria-hidden="true">
          <view class="nx-battery__fill" />
        </view>
      </view>
    </view>

    <!-- Header brand row — TAB routes only (sub-pages carry their own back row) -->
    <view v-if="isTabRoute" class="nx-header" :class="{ 'nx-header--scrolled': chromeScrolled }" :style="{ top: STATUS_BAR_H + 'px' }">
      ...
      <view class="nx-header__r">
        <view class="nx-icon-btn" @click="goSearch">
          <svg width="25.2" height="25.2" viewBox="0 0 24 24" fill="none" stroke="var(--v5-ink-2)" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>
        </view>
        <view class="nx-icon-btn nx-bell" @click="goNotifications">
          <svg width="25.2" height="25.2" viewBox="0 0 24 24" fill="none" stroke="var(--v5-ink-2)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 1 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></svg>
    ...
    <view v-if="!isTabRoute && navHeader" class="nx-navheader" :class="{ 'nx-header--scrolled': chromeScrolled }" :style="{ top: STATUS_BAR_H + 'px', height: navHeaderH + 'px' }">
...
function onContentScroll(e: Event) {
  ...
  chromeScrolled.value = (el?.scrollTop ?? currentScrollTop()) > 4;
}
...
const navHeaderH = computed(() => (navHeader.value ? HEADER_H : 0));
function navBack() {
...
const TABBAR_INSET = TABBAR_H + HOME_INDICATOR_H;
const SUB_BOTTOM = HOME_INDICATOR_H;
const contentTop = computed(() => STATUS_BAR_H + (isTabRoute.value ? HEADER_H : navHeaderH.value));
...
.nx-chassis {
  ...
  height: min(874px, calc(100vh - 32px));
  margin: max(16px, calc((100vh - 874px) / 2)) auto;
  border-radius: 34px;
  overflow: hidden;
  transform: translateZ(0);
  box-shadow: 0 24px 70px rgba(20, 16, 8, 0.18);
}
.nx-statusbar-space {
  position: fixed;
  top: 0;
...
.nx-statusbar-time {
  font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", var(--font-v5), system-ui, sans-serif;
  font-size: 15px;
  font-weight: 600;
  line-height: 1;
...
.nx-signal i:nth-child(1) {
  height: 4px;
}
.nx-signal i:nth-child(2) {
  height: 6px;
}
.nx-signal i:nth-child(3) {
  height: 9px;
}
.nx-signal i:nth-child(4) {
  height: 12px;
}
.nx-wifi {
  display: block;
...
:global(html:not([data-theme="dark"])) .nx-header--scrolled {
  box-shadow:
    0 10px 24px rgba(24, 84, 180, 0.2);
}
...
/* Page nav header (sub-pages) — its OWN chrome surface so the brand row
   ...
   Header nav row: 52 tall, back + centered title + bell, frosted. */
.nx-navheader {
  position: absolute;
  left: 0;
  right: 0;
...
.nx-page-enter :deep(.px-4) {
  padding-left: var(--nx-page-x);
  padding-right: var(--nx-page-x);
}
.nx-page-enter :deep(.mx-4) {
  margin-left: var(--nx-page-x);
  margin-right: var(--nx-page-x);
}
.nx-page-enter :deep(.px-5) {
  padding-left: 24px;
  padding-right: 24px;
}
.nx-page-enter :deep(.mx-5) {
  margin-left: 24px;
  margin-right: 24px;
}
.nx-page-enter :deep(.space-y-3 > :not([hidden]) ~ :not([hidden])) {
  --un-space-y-reverse: 0;
  margin-top: calc(var(--nx-card-gap) * calc(1 - var(--un-space-y-reverse)));
  margin-bottom: calc(var(--nx-card-gap) * var(--un-space-y-reverse));
}
.nx-page-enter :deep(.space-y-4 > :not([hidden]) ~ :not([hidden])) {
  --un-space-y-reverse: 0;
  margin-top: calc(16px * calc(1 - var(--un-space-y-reverse)));
  margin-bottom: calc(16px * var(--un-space-y-reverse));
}
.nx-page-enter :deep(.space-y-6 > :not([hidden]) ~ :not([hidden])) {
  --un-space-y-reverse: 0;
  margin-top: calc(var(--nx-module-gap) * calc(1 - var(--un-space-y-reverse)));
  margin-bottom: calc(var(--nx-module-gap) * var(--un-space-y-reverse));
}
...
.nx-tabbar-pill {
  ...
  border: 1px solid var(--v5-tabbar-border);
  box-shadow:
