          ...
          <view class="md-row" @click="toggle(n)">
            ...
            <view class="md-row-body">
              <view class="md-row-titlewrap">
                <text class="md-row-title" :style="{ color: n.readAt ? 'var(--md-read-title)' : 'var(--v5-ink)' }">{{ n.title }}</text>
                <view v-if="!n.readAt" class="md-unread-dot" />
                <view class="md-chevron" :class="{ 'md-chevron--open': expandedId === n.id }">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" :stroke="n.readAt ? 'var(--md-read-chevron)' : 'var(--v5-ink-3)'" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6" /></svg>
                </view>
              </view>
              <text v-if="n.body" class="md-row-preview" :style="{ color: n.readAt ? 'var(--md-read-preview)' : 'var(--v5-ink-3)' }">{{ n.body }}</text>
              <text class="md-row-time" :style="{ color: n.readAt ? 'var(--md-read-time)' : 'var(--v5-ink-4)' }">{{ timeAgo(n.ts) }}</text>
            </view>
          </view>
...
.md-root {
  ...
  inset: 0;
  z-index: 110;
  --md-backdrop-bg: rgba(19, 20, 26, 0.18);
  --md-read-title: color-mix(in srgb, var(--v5-ink) 48%, transparent);
  --md-read-preview: color-mix(in srgb, var(--v5-ink-3) 58%, transparent);
  --md-read-time: color-mix(in srgb, var(--v5-ink-4) 72%, transparent);
  --md-read-chevron: var(--v5-ink-4);
}
.md-backdrop {
  position: absolute;
  inset: 0;
  background: var(--md-backdrop-bg);
...
<style>
html[data-theme="dark"] .md-root {
  --md-backdrop-bg: rgba(0, 0, 0, 0.6);
}
</style>
