...
<template>
  ...
  <view class="flex items-center justify-between" :style="{ margin: hasLink ? '12px 2px 2px' : '22px 2px 12px' }">
    ...
    <!-- 《07》tap≥44 + 《08》§2:这个链接全站 me 域复用,原先热区只有文字高、按下也零反馈。
         ...
         往那边扩不碰任何东西,右边缘则原地不动。 -->
    <view v-if="link && linkLabel" class="inline-flex items-center gap-0.5 active:opacity-60 transition-opacity" style="font-family: var(--font-v5); font-weight: 500; font-size: 13px; color: var(--v5-ink-3); min-height: 44px; padding-left: 20px" @click="go">
      <text>{{ linkLabel }}</text>
