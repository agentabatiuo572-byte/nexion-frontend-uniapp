  ...
  <view class="mx-4 mt-3 relative overflow-hidden" :style="cardStyle">
    ...
    <view class="relative">
      ...
      <view class="flex items-center justify-between w-full active:opacity-85" :style="ctaStyle" @click="goStaking">
        <text>{{ ctaText }}</text>
        <svg style="color: var(--v5-on-brand-2)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
      </view>
    </view>
    <text class="block" style="margin-top: 8px; font-size: 10.5px; color: var(--v5-ink-4); line-height: 1.375">{{ w.disclaimer }}</text>
  </view>
</template>
