      ...
      <view v-for="(sec, si) in sections" :key="si" :style="sectionStyle(si)">
        ...
        >
          ...
          >
            ...
            <text v-if="row.hint" style="color: var(--v5-ink-4); font-size: 10.5px">({{ row.hint }})</text>
        ...
        <view v-if="sec.checks" style="margin-top: 4px">
          <view v-for="chk in sec.checks" :key="chk" class="flex items-center" style="gap: 8px; padding: 2px 0">
            ...
            <text style="margin-left: auto; font-size: 10.5px; color: color-mix(in srgb, var(--v5-tech-cyan) 70%, transparent)">passed</text>
          </view>
        </view>
      </view>
