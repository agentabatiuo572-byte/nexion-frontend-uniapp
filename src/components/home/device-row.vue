...
-->
<template>
  <view class="block" :style="{ padding: '14px 0', borderBottom: divider ? '1px solid var(--v5-border)' : 'none' }" @click="handleTap">
    <view class="grid items-center gap-3" :style="{ gridTemplateColumns: isPhone ? '48px 1fr auto' : '44px 1fr auto' }">
      <!-- Icon + online dot -->
      <view class="grid place-items-center relative" :style="iconBoxStyle">
        <image v-if="iconKind === 'smartphone'" src="/static/img/devices/real-phone-ui.png" mode="aspectFit" style="width: 48px; height: 48px; display: block" />
        ...
        <view v-if="isOnline" :style="onlineDotStyle" />
      </view>
      ...
      <view class="min-w-0">
        <view class="flex items-center gap-1.5">
          ...
          <text v-if="isOnline" class="font-mono-tabular" style="font-size: 10.5px; padding: 1px 5px; border-radius: 4px; background: var(--v5-success-soft); color: var(--v5-success); font-weight: 500">{{ t.home.deviceEarning }}</text>
        </view>
      ...
      <view class="tabular-nums text-right whitespace-nowrap">
        ...
        <view class="flex items-center justify-end gap-1">
          ...
          <svg v-if="isPhone && isOnline" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--v5-ink-3)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">
            <path :d="taskOpen ? 'M19 15l-7-7-7 7' : 'M5 9l7 7 7-7'" />
          </svg>
        </view>
      </view>
    </view>
    ...
    <view v-if="isPhone && isOnline && taskOpen" class="mt-3" style="padding: 10px 12px; background: linear-gradient(180deg, #111317 0%, #15181C 100%); border-radius: 10px">
...
const onlineDotStyle = computed<CSSProperties>(() => ({
  position: "absolute",
  bottom: "-2px",
  right: "-2px",
  width: "10px",
...
function handleTap() {
  if (isPhone.value && isOnline.value) {
    taskOpen.value = !taskOpen.value;
    return;
  }
  const url = `/pages/store/detail?id=${props.device.kind}`;
