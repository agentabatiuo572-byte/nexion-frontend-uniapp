...
<template>
  <view>
    <view class="flex items-center justify-between" style="margin: 8px 2px 10px">
      <text style="font-family: var(--font-v5); font-weight: 600; font-size: 15px; color: var(--v5-ink); letter-spacing: -0.012em">{{ t.home.onGridTitle }}</text>
      <text class="font-mono-tabular" style="font-size: 13px; color: var(--v5-brand); font-weight: 500" @click="goGlobe">{{ t.home.onGridMap }} →</text>
    </view>

    <view style="background: var(--v5-surface-bg); border-radius: 16px; overflow: hidden">
      <view
        v-for="(c, i) in GRID_CLIENTS"
        :key="c.id"
        class="grid items-center gap-3 px-4 py-2.5"
        :style="{ gridTemplateColumns: '44px 1fr auto', borderBottom: i < GRID_CLIENTS.length - 1 ? '1px solid var(--v5-border)' : 'none' }"
      >
        <view class="grid place-items-center" style="width: 44px; height: 44px; border-radius: 10px; background: var(--v5-brand-soft)">
