  ...
  <view class="grid grid-cols-4 gap-2">
    ...
    >
      <view class="grid place-items-center" style="height: 24px">
        ...
        <!-- 任务 — target (lucide) -->
        <svg v-else-if="c.icon === 'target'" width="24" height="24" viewBox="0 0 24 24" fill="none" :stroke="iconColor(c.tone)" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="12" cy="12" r="10" />
          <circle cx="12" cy="12" r="6" />
          <circle cx="12" cy="12" r="2" />
        </svg>
        <!-- 签到 -->
        <svg v-else width="24" height="24" viewBox="0 0 24 24" fill="none" :stroke="iconColor(c.tone)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <text class="block font-mono-tabular" :style="{ fontSize: '10.5px', color: iconColor(c.tone), marginTop: '1px' }">{{ c.sub }}</text>
    </view>
  </view>
</template>
