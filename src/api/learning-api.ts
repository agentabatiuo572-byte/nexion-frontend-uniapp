...
function course(value: unknown): LearningCourse {
  ...
  return { id: text(row.id), title: text(row.title), body: text(row.body), category: text(row.category), format: text(row.format), level: text(row.level), duration: text(row.duration), rewardNex: row.rewardNex as string | number, featured: row.featured, version: text(row.version), progress: integer(row.progress), completed: row.completed, questions: row.questions.map((value) => { const item = record(value); if (!Array.isArray(item.options)) return invalid(); return { questionId: text(item.questionId), question: text(item.question), options: item.options.map(text) }; }) };
}
...
function result(value: unknown): LearningResult { const row = record(value); if (typeof row.passed !== "boolean" || typeof row.completed !== "boolean" || typeof row.rewardGranted !== "boolean") return invalid(); return { courseId: text(row.courseId), version: text(row.version), score: integer(row.score), passed: row.passed, completed: row.completed, rewardGranted: row.rewardGranted, rewardNex: row.rewardNex as string | number, attempts: integer(row.attempts, 1) }; }
