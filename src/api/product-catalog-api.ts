...
function product(value: unknown): Product {
  ...
  const phase = optionalString(source.unlocksAtPhase);
  if (phase && !PHASES.has(phase as PhaseId)) return invalid();
  ...
  return {
    ...
    unlocksAtPhase: phase as PhaseId | undefined,
...
export function createProductCatalogApi(client: ApiClient): ProductCatalogApi {
  return {
    async catalog(): Promise<ProductCatalogSnapshot> {
      ...
      if (payload.revision !== null && typeof payload.revision !== "string") return invalid();
      if (!Array.isArray(payload.products)) return invalid();
      return {
        source,
