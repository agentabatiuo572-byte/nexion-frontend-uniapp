...
export interface PaymentMethodCard { tokenId: string; brand: "visa" | "mastercard" | "amex" | "unionpay" | "unknown"; last4: string; holder: string; status: "BOUND"; isDefault: boolean; boundAt: string; }
export interface PaymentMethodApi { list(): Promise<PaymentMethodCard[]>; bind(input: { providerToken: string; brand: PaymentMethodCard["brand"]; last4: string; holder: string; makeDefault: boolean; idempotencyKey: string }): Promise<PaymentMethodCard>; }
