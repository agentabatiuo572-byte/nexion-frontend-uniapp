...
function parse(value: unknown, mode: ApiEnvironment): NetworkRankSnapshot {
  const source = record(value);
  const currentRank = nullableInteger(source.currentRank, 1);
  const delta = nullableInteger(source.rankChange24h);
  if (source.serverCanonical !== true || !matchesRuntimeProvenance(source, mode, "nx_user_device")
  ...
  return { source: "nx_user_device", sourceEnvironment: source.sourceEnvironment, runId: source.runId,
    serverCanonical: true, currentRank, rankChange24h: delta,
...
export function createNetworkRankApi(client: ApiClient, mode: ApiEnvironment = "prod"): NetworkRankApi {
  return { snapshot: async () => parse(await client.request({ path: "/api/app/network/rank" }), mode) };
}
