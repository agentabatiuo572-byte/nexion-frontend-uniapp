...
function api(payload: unknown, mode: "remote" | "sandbox") {
...
describe("order list provenance", () => {
  it("remote mode accepts only server production data", async () => {
    await expect(api({ source: "server", sourceEnvironment: "PRODUCTION", runId: null, serverCanonical: true, orders: [order()] }, "remote").list())
  ...
  it("remote mode rejects sandbox/mock data", async () => {
    ...
    await expect(api({ source: "mock", sourceEnvironment: "SANDBOX", runId: "run-20260817", serverCanonical: true, orders: [order()] }, "remote").list())
  ...
  it("sandbox mode requires mock SANDBOX data from the current RunID", async () => {
    setCurrentCommerceSandboxRun("run-20260817");
    await expect(api({ source: "mock", sourceEnvironment: "SANDBOX", runId: "run-20260817", serverCanonical: true, orders: [order()] }, "sandbox").list())
      .resolves.toMatchObject({ source: "mock", sourceEnvironment: "SANDBOX", runId: "run-20260817", serverCanonical: true });
    await expect(api({ source: "server", sourceEnvironment: "PRODUCTION", runId: null, serverCanonical: true, orders: [order()] }, "sandbox").list())
      .rejects.toMatchObject({ message: "ORDER_RESPONSE_INVALID" });
  });

  it("sandbox mode rejects a stale RunID", async () => {
    setCurrentCommerceSandboxRun("run-20260817");
    await expect(api({ source: "mock", sourceEnvironment: "SANDBOX", runId: "run-20260816", serverCanonical: true, orders: [order()] }, "sandbox").list())
      .rejects.toMatchObject({ message: "ORDER_RESPONSE_INVALID" });
  });

  it("rejects an order envelope without the server canonical marker", async () => {
    await expect(api({ source: "server", sourceEnvironment: "PRODUCTION", runId: null, orders: [order()] }, "remote").list())
      .rejects.toMatchObject({ message: "ORDER_RESPONSE_INVALID" });
    await expect(api({ source: "server", sourceEnvironment: "PRODUCTION", runId: null, serverCanonical: false, orders: [order()] }, "remote").list())
      .rejects.toMatchObject({ message: "ORDER_RESPONSE_INVALID" });
  });
});
