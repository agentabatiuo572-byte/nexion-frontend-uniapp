...
import { V_RANKS, type RankGap } from "@/store/v-rank";
import { fmt } from "@/i18n/format";

/**
 * V 级头衔的显示名。中文界面用 `cnTitle`(主人 2026-08-17 拍板:V3 = 舰长),其余语言用英文权威名
 * (`title`;越南语没有独立头衔表)。此前这条判断在 `network-card.vue` / `rank-how.vue` 各写一份、
 * 而 V 级页阶梯与领导池压根没判 —— 于是同一个头衔在中文界面有「舰长 / 队长 / Captain」三种写法。
 * 收成一处,谁渲染头衔都从这里取。
 */
export function rankTitle(v: number, isZh: boolean): string {
  const def = V_RANKS[v];
  if (!def) return "";
  return (isZh && def.cnTitle) || def.title;
}

/** `V3 舰长` / `V3 Captain` —— 阶梯行、解锁句、进度头都用这个拼法。 */
export function rankLabel(v: number, isZh: boolean): string {
  const title = rankTitle(v, isZh);
  return title ? `V${v} ${title}` : `V${v}`;
}

/** 一条晋升缺口 → 当前语言的一句话。 */
export function rankGapText(t: Messages, gap: RankGap, isZh: boolean): string {
  switch (gap.kind) {
    case "selfBuy":
    ...
    case "vDownlines":
      return fmt(t.rank.needV, { n: gap.n, title: rankTitle(gap.vLevel, isZh), v: gap.vLevel });
  }
}
