    ...
    <view style="padding-bottom: 24px">
      ...
      <view class="px-4" style="display: flex; flex-direction: column; gap: 12px">
        ...
        <view class="nx-repurchase-submit-cta w-full flex items-center justify-center" :class="{ 'active:scale-[0.98]': canSubmit }" :style="ctaStyle" @click="handleRepurchase">
          <text>{{ ctaLabel }}</text>
          <svg :style="{ color: canSubmit ? 'var(--v5-on-brand)' : 'var(--v5-ink-4)', marginLeft: '8px' }" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
        </view>
...
const heroStyle: CSSProperties = {
  ...
  padding: "16px",
  background:
    "radial-gradient(80% 60% at 50% 0%, color-mix(in srgb, var(--v5-brand) 18%, transparent) 0%, transparent 65%), var(--v5-surface-bg)",
