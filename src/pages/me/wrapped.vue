<!--
  ...
  navBack() helper (stack-aware: pops history, else → /pages/me/me on cold).
-->
<template>
    ...
    <view :style="overlayStyle" @click="next">
        ...
        <view :key="idx" class="nx-story-in" :style="cardStyle">
          <view style="padding: 0 8px; text-align: left">
            ...
            <template v-else-if="idx === 1">
              ...
              <text class="block" :style="card2BodyStyle">{{ card2BodyText }}</text>
            </template>
            <!-- 2: devices -->
      ...
      <view class="flex items-center justify-between shrink-0" :style="bottomRowStyle">
        ...
        <view class="grid place-items-center active:opacity-70" :style="navBtnStyle(idx === CARDS - 1)" @click.stop="next">
          <svg style="color: var(--v5-ink-3)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
        </view>
      </view>
    </view>
  </AppChassis>
...
const labelStyle: CSSProperties = { fontSize: "10.5px", letterSpacing: "0.16em", color: "var(--v5-ink-3)" };
...
// card2 "total earned" body uses mt-4 (16px) in the prototype, unlike the
// mt-3 (12px) bodies on cards 1/3/4 — the shared body14Style stays 12px and
// card2 gets its own marginTop to match the source 1:1.
const card2BodyStyle: CSSProperties = {
  marginTop: "16px",
  fontSize: "14px",
  color: "color-mix(in srgb, var(--v5-ink) 80%, transparent)",
  lineHeight: 1.625,
...
function kickerStyle(color: string): CSSProperties {
  return { fontSize: "10.5px", letterSpacing: "0.18em", color };
}
