...
const locale = useLocaleStore(); const courseId = ref(""); const course = ref<LearningCourse | null>(null); const result = ref<LearningResult | null>(null); const answers = ref<number[]>([]); const loading = ref(true); const submitting = ref(false); const error = ref(""); const quizKey = ref("");
...
const submitCourse = createSingleFlight(async () => { if (!course.value) return null; return course.value.questions.length ? learningApi.submitQuiz(course.value.id, answers.value, quizKey.value) : learningApi.complete(course.value.id); });
async function finish() { if (!course.value || loading.value || submitting.value) return; if (course.value.questions.length && answers.value.some((answer) => !Number.isSafeInteger(answer) || answer < 0)) { error.value = "请先完成全部题目后再提交。"; return; } submitting.value = true; error.value = ""; try { const submitted = await submitCourse(); if (submitted) { result.value = submitted; return; } if (!(await recoverAuthoritative())) error.value = "提交结果尚未确认；已重新读取课程进度，奖励不会按本地状态显示。"; } catch { try { if (await recoverAuthoritative()) return; } catch {} error.value = "提交结果尚未确认；已重新读取课程进度，奖励不会按本地状态显示。"; } finally { submitting.value = false; } }
