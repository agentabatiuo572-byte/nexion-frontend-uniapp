      ...
      <view class="px-4" style="display: flex; flex-direction: column; gap: 12px">
        ...
        <view class="grid grid-cols-3" style="gap: 8px">
          ...
          >
            ...
            <text class="block" :style="{ fontSize: '10.5px', color: 'var(--v5-ink-3)', marginTop: '2px' }">{{ t.commissions.kind[k] }}</text>
          ...
          <template v-else>
            ...
            >
              ...
              <view class="text-right">
                ...
                <text v-if="e.amountNEX > 0" class="block font-mono-tabular tabular-nums" :style="{ fontSize: '10.5px', color: 'var(--v5-warning)' }">+{{ e.amountNEX.toLocaleString() }} NEX</text>
                <text v-if="e.status === 'cooling'" class="block" :style="{ fontSize: '10.5px', color: 'var(--v5-warning)', marginTop: '2px' }">{{ coolingTag(e) }}</text>
                <text v-else-if="e.status === 'unlocked'" class="block" :style="{ fontSize: '10.5px', color: 'var(--v5-success)', marginTop: '2px' }">{{ t.commissions.readyTag }}</text>
                <text v-else-if="e.status === 'withdrawn'" class="block" :style="{ fontSize: '10.5px', color: 'var(--v5-ink-3)', marginTop: '2px' }">{{ t.commissions.withdrawnTag }}</text>
              </view>
            </view>
          </template>
        </view>
...
function kindCardStyle(k: CommissionKind): CSSProperties {
  ...
  return {
    padding: "10px",
    borderRadius: "12px",
    background: active ? "color-mix(in srgb, var(--v5-surface-2) 60%, transparent)" : "var(--v5-surface-bg)",
...
const eventMetaStyle: CSSProperties = { fontSize: "10.5px", color: "var(--v5-ink-3)", marginTop: "2px" };
</script>
