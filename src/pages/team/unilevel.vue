    ...
    <view class="pb-6" style="color: var(--v5-ink)">
      ...
      <view class="px-4 flex items-center justify-end" style="padding-top: 8px; padding-bottom: 8px">
        <view class="nx-unilevel-how-link inline-flex items-center active:scale-[0.98]" :style="howEntryStyle" @click="go('/pages/team/unilevel-how')">
          ...
          <svg style="color: var(--v5-brand-2)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
        </view>
      </view>
      ...
      <view class="px-4" style="display: flex; flex-direction: column; gap: 12px">
        ...
        <view class="rounded-2xl" :style="heroStyle">
          <text class="block font-mono-tabular" :style="{ fontSize: '10.5px', letterSpacing: '0.16em', color: 'var(--v5-brand)' }">{{ t.unilevel.heroLabel }}</text>
        ...
        <view class="rounded-2xl" :style="{ padding: '16px', background: 'var(--v5-success-soft)' }">
          <view class="flex items-start" style="gap: 12px">
            ...
            <view class="text-right shrink-0">
              ...
              <text class="block" :style="{ fontSize: '10.5px', color: 'var(--v5-ink-3)', marginTop: '2px' }">{{ directMembersText }}</text>
            </view>
          </view>
        </view>
        ...
        <view class="rounded-2xl" :style="{ padding: '16px', background: 'var(--v5-tech-cyan-soft)' }">
          ...
          <view class="flex items-start" :style="algoNoteStyle">
            ...
            <text :style="{ fontSize: '10.5px', color: 'var(--v5-ink-3)', lineHeight: 1.625 }">{{ t.unilevel.networkAlgoNote }}</text> <!-- SKILL: leading-relaxed=1.625 -->
          </view>
        </view>
        ...
        <view class="rounded-2xl border" :style="{ padding: '16px', background: 'var(--v5-surface-bg)', borderColor: 'var(--v5-border)' }">
          ...
          <view class="grid grid-cols-4" style="margin-top: 12px; gap: 6px">
            <view v-for="tier in RATE_TIERS" :key="tier.id" class="rounded-lg text-center" :style="tierCardStyle(tier)">
              ...
              <text class="block" :style="{ fontSize: '10.5px', marginTop: '2px', lineHeight: 1.25, color: tier.id === currentTier.id ? tier.color : 'var(--v5-ink-4)' }">{{ t.unilevel.rateTiers[tier.id].perk }}</text> <!-- SKILL: leading-tight=1.25 (was 1.1) -->
              <text class="block font-mono-tabular" :style="{ fontSize: '10.5px', color: 'var(--v5-ink-4)', marginTop: '2px' }">{{ tierVolLabel(tier.minVolume) }}</text>
            </view>
          </view>
...
const heroBigStyle: CSSProperties = { marginTop: "6px", fontFamily: "var(--font-amount)", fontSize: "30px", fontWeight: 600, lineHeight: 1, color: "var(--v5-ink)" };
...
function tierCardStyle(tier: RateTier): CSSProperties {
  ...
  return {
    ...
    border: `1px solid ${isCurrent ? `color-mix(in srgb, ${tier.color} 55%, transparent)` : "color-mix(in srgb, var(--v5-border) 70%, transparent)"}`,
  };
}
