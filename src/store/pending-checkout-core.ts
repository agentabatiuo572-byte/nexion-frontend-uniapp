...
/** 确认页那份报价快照 —— 扣款只认它(展示与扣款同源),恢复会话时原样复位。 */
...
export function normalizeSessions(raw: unknown): PendingCheckoutSession[] {
  ...
  for (const r of raw) {
    ...
    out.push({
      ...
      quote: {
        total: q.total,
        voucher: { id: typeof q.voucher?.id === "string" ? q.voucher.id : null, discount: Number(q.voucher?.discount) || 0 },
        trial: {
          applied: q.trial?.applied === true,
          promo: Number(q.trial?.promo) || 0,
          offsetUSD: Number(q.trial?.offsetUSD) || 0,
          remainderUSD: Number(q.trial?.remainderUSD) || 0,
          shadowNEX: Number(q.trial?.shadowNEX) || 0,
        },
